...
Texture createTexture(VulkanBackend& backend, void* data, uint32_t size, uint32_t width, uint32_t height, bool generateMips)
{
    ...
    backend.immediateSubmit([&](VkCommandBuffer cmd)
    {
        VkImageMemoryBarrier imageMemoryBarrierForTransfer = vkutil::init::imageMemoryBarrier(VK_IMAGE_LAYOUT_UNDEFINED,
                                                                                              VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, texture.image.image, 0, VK_ACCESS_TRANSFER_WRITE_BIT, texture.mipCount);
    ...
    VkImageViewCreateInfo imageViewInfo = vkutil::init::imageViewCreateInfo(VK_FORMAT_R8G8B8A8_SRGB, texture.image.image, VK_IMAGE_ASPECT_COLOR_BIT, texture.mipCount);
